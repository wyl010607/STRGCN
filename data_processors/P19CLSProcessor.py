...
class P19CLSProcessor(AbstractDataProcessor):
    """
    P19 classification processor (PT_dict_list_6-based):
    Read directly from  P19 processed_data/PT_dict_list_6.npy and processed_data/arr_outcomes_6.npy
    ...
    def __init__(self, *args, split_idx: int = 1, **kwargs):
        super().__init__("P19", *args, **kwargs)
    ...
    def _load_ptdict_list_and_labels(self) -> Tuple[np.ndarray, np.ndarray]:
        Pdict_list = np.load(self.processed_dir / "PT_dict_list_6.npy", allow_pickle=True)
        arr_outcomes = np.load(self.processed_dir / "arr_outcomes_6.npy", allow_pickle=True)
    ...
    def _load_official_split(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        idx_train, idx_val, idx_test = np.load(
            self.splits_dir / f"phy19_split{self.split_idx}_new.npy",
            allow_pickle=True
        )
    ...
    def process_classification(self):
        ...
        # 9) log
        print(f"P19 total N={N_total} | split sizes: "

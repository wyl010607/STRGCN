    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.learnable_adj_weight_U = nn.Embedding(
            num_embeddings=num_nodes, embedding_dim=node_dim
        )
        ...
        self.bottom_weight_S_Embedding = nn.Embedding(
            num_embeddings=num_nodes, embedding_dim=in_dim
        )
        self.middle_weight_S_Embedding = nn.ModuleList(
            [
                nn.Embedding(num_embeddings=num_nodes, embedding_dim=out_dim)
                for _ in range(gcn_depth)
            ]
        )
        self.top_weight_S_Embedding = nn.Embedding(
            num_embeddings=num_nodes, embedding_dim=out_dim
        )
        self.bottom_bias = (
